...
echo Generating session resource...
call npx @nestjs/cli g resource sessions --no-spec
if %ERRORLEVEL% NEQ 0 (
  echo ERROR: Failed to generate resource.
  goto :error
)

echo Done! The sessions module has been created with controller, service, DTOs, and entity.
goto :end
