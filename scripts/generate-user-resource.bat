...
echo Generating user resource...
call npx @nestjs/cli g resource users --no-spec
if %ERRORLEVEL% NEQ 0 (
  echo ERROR: Failed to generate resource.
  goto :error
)

echo Done! The users module has been created with controller, service, DTOs, and entity.
goto :end
